 ...
 * `SectionDefinition`, dispatching to the matching `SectionFieldProvider`.
 *
 * @template-extends AbstractType<LayoutValuesDto>
 * @template-implements DataMapperInterface<LayoutValuesDto>
 */
class SectionFormType extends AbstractType implements DataMapperInterface
{
    public function __construct(
        private readonly SectionFieldProviderRegistry $registry,
    ) {
    }

    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setRequired('section_definition');
        $resolver->setAllowedTypes('section_definition', SectionDefinition::class);
    ...
    {
        /** @var SectionDefinition $definition */
        $definition = $options['section_definition'];
        ...
        $builder->setAttribute('section_definition', $definition);
     ...
     */
    private function resolveDefinition(array $forms): ?SectionDefinition
    {
        $first = reset($forms);
        if ($first === false) {
            return null;
        ...
        $definition = $parent->getConfig()->getAttribute('section_definition');
