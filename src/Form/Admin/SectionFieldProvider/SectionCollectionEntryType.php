 ...
 *              matching member's SectionFieldProvider — PRE_SET_DATA
 *              picks based on current data, PRE_SUBMIT picks based on
 *              submitted type so validation uses the right field).
 *   - scalar → single `value` built via the matching SectionFieldProvider
 *
 ...
 */
class SectionCollectionEntryType extends AbstractType
{
    ...
    private function buildUnionEntry(FormBuilderInterface $builder, BlockItemOptionUnion $union): void
    {
        ...
        $builder->add('type', ChoiceType::class, [
            ...
            'expanded' => true,
            'multiple' => false,
            'block_prefix' => 'xutim_section_union_type',
        ]);
