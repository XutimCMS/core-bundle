...
/**
 * Builds a single form field inside a `SectionFormType` for a given
 * `BlockItemOption` type. Unlike `BlockItemProviderInterface`, field
 * names are provided by the caller (the `SectionDefinition`), so the
 * same option type can be used for multiple fields in one form.
 */
interface SectionFieldProviderInterface
{
    /**
