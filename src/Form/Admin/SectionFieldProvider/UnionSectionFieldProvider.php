 ...
 */
final readonly class UnionSectionFieldProvider implements SectionFieldProviderInterface
{
    ...
    public function buildField(
        ...
        if (!$option instanceof BlockItemOptionUnion) {
            throw new \LogicException('UnionSectionFieldProvider requires BlockItemOptionUnion');
        }

        if (!$builder instanceof FormBuilderInterface) {
            // Union inside a union (nested) is not a practical case; the
            // event-listener path in SectionCollectionEntryType only invokes
            // providers for scalar members (Page, Article, Text, …).
            throw new \LogicException('UnionSectionFieldProvider can only be used at form build time (FormBuilderInterface), not inside event listeners.');
        }
        ...
        $sub = $builder->create($fieldName, FormType::class, [
            ...
            'required' => false,
            'block_prefix' => 'xutim_section_union',
            'by_reference' => false,
        ]);
