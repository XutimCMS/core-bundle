...
class XutimSectionExtensionRuntime implements RuntimeExtensionInterface
{
    public function __construct(
        private readonly SectionDefinitionRegistry $registry,
        private readonly XutimSectionValueResolver $resolver,
    ...
    /**
     * Render a xutimSection editor.js fragment. Accepts the block's
     * `data` payload (`{sectionCode, values}`) as an associative array.
     ...
     *
     * Twig component scope footgun for downstream section templates.
     ...
     * to invoke a method on a string variable". Use section-specific
    ...
    public function render(array $data, bool $editable = false): string
    {
        ...
        if (!is_string($sectionCode) || $sectionCode === '') {
            return '';
        }

        $definition = $this->registry->getByCode($sectionCode);
        if ($definition === null) {
            $this->logger->warning('Unknown xutim section code in rendered content', [
                'code' => $sectionCode,
            ]);
        ...
        try {
            ...
            $this->logger->warning('Failed to render xutim section', [
                'code' => $sectionCode,
            ...
            // returning '' — a half-broken section shouldn't leak stack
    ...
    private function renderEditableError(SectionDefinition $definition, \Throwable $e): string
    {
        return sprintf(
            ...
            . 'line-height:1.5">'
            . '<div style="font-weight:600;margin-bottom:6px">xutim section render failed</div>'
     ...
     *
     * Multiline is auto-detected from the section's field definition (any
    ...
    public function editable(array $context, string $field, mixed $value, ?string $placeholder = null): string
    {
        ...
        $fieldOption = $section instanceof SectionDefinition ? ($section->getFields()[$field] ?? null) : null;
    ...
    /**
     * Opening marker for a block rendered automatically by the section
