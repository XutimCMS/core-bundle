...
/**
 * Presents raw xutimSection field values as display structs for the
 ...
 *
 * Display struct shape consumed by xutim_section_diff.html.twig:
 ...
 */
class XutimSectionDiffExtensionRuntime implements RuntimeExtensionInterface
{
    public function __construct(
        ...
        private readonly XutimSectionExtensionRuntime $sectionRuntime,
        private readonly SiteContext $siteContext,
    ) {
    }

    public function sectionName(string $code): string
    {
     ...
     */
    public function fieldDisplay(string $sectionCode, string $fieldName, mixed $raw, string $locale): array
    {
        if ($raw === null || $raw === '' || $raw === []) {
        ...
        if ($option instanceof RichTextBlockItemOption) {
            if (is_array($raw)) {
                return ['kind' => 'html', 'text' => $this->sectionRuntime->richTextHtml($raw)];
            }
