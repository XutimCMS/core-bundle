...
final class XutimSectionDiffExtensionRuntimeTest extends TestCase
{
    private const PAGE_ID = '784305be-175d-4015-bf70-ce60ef40d34b';
    private const MEDIA_ID = 'e39c3d40-0efd-4cbf-9f8a-eb343b4c05f8';

    public function testSectionNameFallsBackToCode(): void
    {
        $runtime = $this->createRuntime([]);

        self::assertSame('unknown-code', $runtime->sectionName('unknown-code'));
    }

    public function testSectionNameUsesDefinitionName(): void
    {
        $runtime = $this->createRuntime(['hero' => new TextBlockItemOption()]);

        self::assertSame('Test', $runtime->sectionName('test'));
    }
    ...
    private function createRuntime(
    ...
    ): XutimSectionDiffExtensionRuntime {
        $logger = $this->createStub(LoggerInterface::class);
        $registry = new SectionDefinitionRegistry($fields === [] ? [] : [$this->sectionWithFields($fields)]);
        ...
        return new XutimSectionDiffExtensionRuntime(
            $registry,
            $resolver,
            $adminEditUrlResolver,
            new XutimSectionExtensionRuntime(
                $registry,
                $resolver,
