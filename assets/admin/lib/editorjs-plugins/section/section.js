/**
 * Editor.js tool for xutimSection compound blocks.
 *
 * Each section is a typed set of fields (text, image, page ref, …) stored
...
export default class XutimSectionTool {
    ...
    constructor({ data, config, api, block, readOnly }) {
        ...
        this.sections = Array.isArray(this.config.sections)
        ...
        this.sectionPickerWrapper = null;
    ...
    render() {
        ...
        this.wrapper.className = 'xutim-section-block my-4';
    ...
    renderBody() {
        ...
        if (!this.data.sectionCode) {
            ...
            if (this.sections.length === 0) {
                ...
                empty.textContent = 'No sections registered';
            ...
            if (!this.readOnly && !this.autoPickerShown) {
                ...
                setTimeout(() => this.openSectionPicker(), 0);
                return;
            }
    ...
    renderPicker() {
        ...
        hint.textContent = 'No section picked yet.';
        picker.appendChild(hint);

        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'btn btn-primary';
        btn.textContent = 'Choose a section';
    ...
    openSectionPicker() {
        if (this.sectionPickerWrapper) return;
        ...
        dialog.appendChild(this.buildSectionPickerContent());
        wrapper.appendChild(dialog);
        document.body.appendChild(wrapper);
        this.sectionPickerWrapper = wrapper;
        dialog.showModal();
        document.body.classList.add('overflow-hidden');

        dialog.addEventListener('close', () => {
            if (wrapper.parentNode) wrapper.parentNode.removeChild(wrapper);
            if (this.sectionPickerWrapper === wrapper)
                this.sectionPickerWrapper = null;
            document.body.classList.remove('overflow-hidden');
        });
    }

    buildSectionPickerContent() {
        const container = document.createElement('div');
        container.className = 'p-4';

        const heading = document.createElement('h3');
        heading.className = 'mb-3';
        heading.textContent = 'Choose a section';
        ...
        const cardWrappers = this.sections.map((section) =>
    ...
    buildSectionCard(section) {
        ...
        col.dataset.category = section.category || 'Other';
        ...
        const hasImage = !!section.previewImage;
        ...
        card.addEventListener('click', (event) => {
            event.preventDefault();
            this.closeSectionPicker();
            this.selectSection(section.code);
        });
    ...
    closeSectionPicker() {
        if (!this.sectionPickerWrapper) return;
        const dialog = this.sectionPickerWrapper.querySelector('dialog');
    ...
    renderPlaceholder() {
        if (this.previewUrl) {
            ...
            previewWrap.className =
                'xutim-section-block__preview position-relative';
            previewWrap.style.overflow = 'hidden';

            const iframe = document.createElement('iframe');
            iframe.className = 'border-0';
            iframe.setAttribute('title', 'Section preview');
    ...
    refreshPreview() {
        if (!this.previewIframe || !this.previewUrl || !this.data.sectionCode)
            return;

        const base = this.buildUrl(this.previewUrl, this.data.sectionCode);
        ...
        fetch(url, {
            ...
            .catch((err) => {
                console.warn('xutim-section preview failed', err);
            });
    }
    ...
    sectionDisplayName() {
        ...
        return section ? section.name : this.data.sectionCode;
    }
    ...
    previewText() {
        ...
        if (!section) return '';
        const textField = (section.fields || []).find((f) => f.translatable);
        if (!textField) return '';
    ...
    openEditor() {
        if (this.readOnly || !this.data.sectionCode) return;
        ...
        // clipped at the dialog edge. If a section form ever gets tall
        ...
        const url =
            this.buildUrl(this.formUrl, this.data.sectionCode) +
            '?values=' +
    ...
    refreshForm(container) {
        if (!this.refreshUrl || !this.data.sectionCode) return;

        const formData = this.collectFormData(container);
        const url = this.buildUrl(this.refreshUrl, this.data.sectionCode);
        ...
        fetch(url, {
            ...
            .catch((err) => {
                console.warn('xutim-section refresh failed', err);
            });
    }
    ...
    bindCollectionHandlers(container) {
        ...
        container
            .querySelectorAll('[data-xutim-section-collection-add]')
            .forEach((btn) => {
                btn.addEventListener('click', (event) => {
                    event.preventDefault();
                    const target = btn.getAttribute(
                        'data-xutim-section-collection-target',
                    );
                    const collection = container.querySelector(
                        `[data-xutim-section-collection][data-xutim-section-collection-prefix="${target}"]`,
                    );
                    ...
                    const index = parseInt(
                        collection.getAttribute(
                            'data-xutim-section-collection-index',
                        ) || '0',
                        10,
                    );
                    ...
                    const wrapper = document.createElement('div');
                    wrapper.className =
                        'xutim-section-collection__item border rounded p-2 mb-2';
                    wrapper.setAttribute(
                        'data-xutim-section-collection-item',
                        '',
                    );
    ...
    submitForm(container) {
        const formData = this.collectFormData(container);
        const url = this.buildUrl(this.saveUrl, this.data.sectionCode);
        ...
        fetch(url, {
            ...
            .catch((err) => {
                console.warn('xutim-section save failed', err);
    ...
    renderSettings() {
        if (this.readOnly || !this.data.sectionCode) return null;
        ...
        items.push({
            ...
            onClick: () => this.openSectionPicker(),
        });
